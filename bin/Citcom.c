...
{ /* Functions called by main*/
  ...
  void update_cmb_T();    // ZYZ-220218
  void output_heatflux(); // ZYZ-220218
  ...
  while (E->control.keep_going && (Emergency_stop == 0)) {
    ...
    heat_flux(E);
    if (E->parallel.me_loc[3] == 0 && E->mush.dynamics.solve_CMB_temp)
      update_cmb_T(E);
    /* deal with surface downwelling */
    if ((E->mush.mush_version) && (E->mush.dynamics.surface_downwelling))
      surface_downwelling(E);
    compute_horiz_avg(E);
    ...
    */
    ...
    if (E->parallel.me == 0) {
      fprintf(E->fp,
              "CPU total = %g & CPU = %g for step %d time = %.4e dt = %.4e  "
              "maxT = %.4e sub_iteration%d\n",
  ...
  if (E->parallel.me == 0) {
    ...
    fprintf(stderr, "Average cpu time taken for velocity step = %f\n",
            cpu_time_on_vp_it /
                ((float) (E->monitor.solution_cycles - E->control.restart)));
