...
void full_parallel_processor_setup(struct All_variables *E) {
  ...
  if (E->parallel.nprocx != E->parallel.nprocy) {
    if (E->parallel.me == 0)
      fprintf(stderr, "!!!! nprocx must equal to nprocy \n");
  ...
  if (total_proc != E->parallel.nproc) {
    if (E->parallel.me == 0)
      fprintf(stderr, "!!!! # of requested CPU is incorrect (expected: %i got: %i)\n",
  ...
  if (E->sphere.caps_per_proc > 1) {
    if (E->parallel.me == 0)
      fprintf(stderr, "!!!! # caps per proc > 1 is not supported.\n \n");
    parallel_process_termination();
  }
  ...
  /* x-direction then*/
  E->parallel.me_loc[1] =
      ((me - cap_id_surf * proc_per_cap - E->parallel.me_loc[3]) / E->parallel.nprocz)
          % E->parallel.nprocx;

  /* y-direction then*/
  E->parallel.me_loc[2] =
      ((((me - cap_id_surf * proc_per_cap - E->parallel.me_loc[3]) / E->parallel.nprocz)
          - E->parallel.me_loc[1]) / E->parallel.nprocx) % E->parallel.nprocy;
...
static void set_horizontal_communicator(struct All_variables *E) {
  ...
  if (E->control.verbose) {
    fprintf(E->fp_out,
            "horizontal group of me=%d loc3=%d\n",
            E->parallel.me,
            E->parallel.me_loc[3]);
...
static void set_vertical_communicator(struct All_variables *E) {
  ...
  if (E->control.verbose) {
    fprintf(E->fp_out,
            "vertical group of me=%d loc1=%d loc2=%d\n",
            E->parallel.me,
            E->parallel.me_loc[1],
            E->parallel.me_loc[2]);
    ...
    for (lev = E->mesh.gridmax; lev >= E->mesh.gridmin; lev--) {
      ...
      for (m = 1; m <= E->sphere.caps_per_proc; m++) {
        fprintf(E->fp_out,
                ...
                E->parallel.me,
                E->sphere.capid[m],
                E->parallel.TNUM_PASS[lev][m]);
        for (k = 1; k <= E->parallel.TNUM_PASS[lev][m]; k++) {
          fprintf(E->fp_out,
                  "proc %d and pass  %d to proc %d with %d eqn and %d node\n",
                  E->parallel.me,
                  k,
                  E->parallel.PROCESSOR[lev][m].pass[k],
                  E->parallel.NUM_NEQ[lev][m].pass[k],
                  E->parallel.NUM_NODE[lev][m].pass[k]);
      ...
      for (k = 1; k <= E->parallel.TNUM_PASSz[lev]; k++) {
        ...
        fprintf(E->fp_out,
                "proc %d and pass  %d to proc %d\n",
                E->parallel.me,
                k,
                E->parallel.PROCESSORz[lev].pass[k]);
        for (m = 1; m <= E->sphere.caps_per_proc; m++) {
          fprintf(E->fp_out,
                  "cap=%d eqn=%d node=%d\n",
                  E->sphere.capid[m],
                  E->parallel.NUM_NEQ[lev][m].pass[kkkp],
                  E->parallel.NUM_NODE[lev][m].pass[kkkp]);
  ...
  if (E->control.verbose) {
    for (lev = E->mesh.gridmax; lev >= E->mesh.gridmin; lev--) {
      ...
      for (m = 1; m <= E->sphere.caps_per_proc; m++) {
        ...
        for (k = 1; k <= E->parallel.TNUM_PASS[lev][m]; k++) {
          fprintf(E->fp_out,
                  "proc %d and pass  %d to proc %d with %d node\n",
                  E->parallel.me,
                  k,
                  E->parallel.PROCESSOR[lev][m].pass[k],
                  E->parallel.NUM_sNODE[lev][m].pass[k]);
          fprintf(E->fp_out, "Node:\n");
...
/* exchange vertical node (ZYZ-220416) */
void exchange_vertical_node_f(struct All_variables *E, float **U, int lev) {
  int j, jj, k, kk, idb, sizeofk;
  float *RV, *SV;
  const int m = 1;
  ...
  idb = 0;
  for (k = 1; k <= E->parallel.TNUM_PASSz[lev]; k++) {
    sizeofk = (1 + E->parallel.NUM_sNODE[lev][m].pass[k]) * sizeof(float);
    idb = max(idb, sizeofk);
  }
  ...
  for (k = 1; k <= E->parallel.TNUM_PASSz[lev]; k++) {
    ...
    for (j = 1; j <= E->parallel.NUM_sNODE[lev][m].pass[kk]; j++)
      SV[jj++] = U[m][E->parallel.EXCHANGE_sNODE[lev][m][j].pass[kk]];

    MPI_Sendrecv(SV, E->parallel.NUM_sNODE[lev][m].pass[k], MPI_FLOAT,
                 E->parallel.PROCESSORz[lev].pass[k], 1,
                 RV, E->parallel.NUM_sNODE[lev][m].pass[k], MPI_FLOAT,
                 E->parallel.PROCESSORz[lev].pass[k], 1, E->parallel.world, &status1);

    jj = 0;
    for (j = 1; j <= E->parallel.NUM_sNODE[lev][m].pass[kk]; j++)
      U[m][E->parallel.EXCHANGE_sNODE[lev][m][j].pass[kk]] += RV[jj++];
  }
