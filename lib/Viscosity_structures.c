...
void read_visc_param_from_file(struct All_variables *E, const char *param,
                               float *var, FILE *fp);
...
void viscosity_system_input(struct All_variables *E) {
  ...
  input_int("allow_anisotropic_viscosity",
            &(E->viscosity.allow_anisotropic_viscosity), "0", m);
#ifndef CITCOM_ALLOW_ANISOTROPIC_VISC
  if (E->viscosity.allow_anisotropic_viscosity) { /* error */
    fprintf(stderr, "error: allow_anisotropic_viscosity is not zero, but code "
                    "not compiled with CITCOM_ALLOW_ANISOTROPIC_VISC\n");
...
anisotropic
viscosity */
    input_int("anisotropic_init", &(E->viscosity.anisotropic_init), "0",
              m); /* 0: isotropic
1: random
...
6: use radially aligned director and taper eta_s/eta from base (1) to top of
layer (ani_vis2_factor)

*/
    input_string("anisotropic_init_dir", (E->viscosity.anisotropic_init_dir),
                 "", m); /* directory
for
ggrd
type
init */
    input_int("anivisc_layer", &(E->viscosity.anivisc_layer), "1",
              m); /* >0: assign to layers on top of anivisc_layer
<0: assign to layer = anivisc_layer
*/
    if ((E->viscosity.anisotropic_init == 6) &&
        (E->viscosity.anivisc_layer >= 0))
      myerror(E, "anisotropic init mode 6 requires selection of layer where "
                 "anisotropy applies");
    ...
    if (!E->viscosity.anivisc_start_from_iso)
      if (E->viscosity.anisotropic_init == 3) {
        if (E->parallel.me == 0)
          fprintf(
              stderr,
              "WARNING: overriding anisotropic first step for ani init mode\n");
  ...
  if (E->viscosity.TDEPV) {
    ...
    input_float("visc_T_ref", &(E->viscosity.T_ref), "1.0", m); // ZYZ-220214
  ...
  if (E->viscosity.SDEPV) {
    ...
    input_float_vector("sdepv_expt", E->viscosity.num_mat,
                       (E->viscosity.sdepv_expt), m);
  }

  input_boolean("PDEPV", &(E->viscosity.PDEPV), "off",
                m); /* plasticity addition by TWB */
  if (E->viscosity.PDEPV) {
    ...
    input_boolean("psrw", &(E->viscosity.psrw), "off",
                  m); /* SRW? else regular plasiticity */
    input_boolean("pdepv_eff", &(E->viscosity.pdepv_eff), "on", m); /* effective
                                 or
                                 min/max
                                 approach */
    input_float_vector("pdepv_a", E->viscosity.num_mat, (E->viscosity.pdepv_a),
                       m);
    input_float_vector("pdepv_b", E->viscosity.num_mat, (E->viscosity.pdepv_b),
                       m);
    input_float_vector("pdepv_y", E->viscosity.num_mat, (E->viscosity.pdepv_y),
                       m);

    input_float("pdepv_offset", &(E->viscosity.pdepv_offset), "0.0", m);
  }
  input_float("sdepv_misfit", &(E->viscosity.sdepv_misfit), "0.001",
              m); /* there should be no harm in having
...
*/
  ...
  if (E->viscosity.CDEPV) {
    ...
    input_float_vector("cdepv_ff", E->trace.nflavors, (E->viscosity.cdepv_ff),
                       m);
  ...
  input_float("lv_channel_thickness", &(E->viscosity.lv_channel_thickness),
              "0.0047", m);
...
void allocate_visc_vars(struct All_variables *E) {
  ...
  if (E->viscosity.N0 == NULL || E->viscosity.E == NULL ||
      E->viscosity.T == NULL || E->viscosity.Z == NULL ||
      E->viscosity.pdepv_a == NULL || E->viscosity.pdepv_b == NULL ||
      E->viscosity.pdepv_y == NULL || E->viscosity.sdepv_expt == NULL) {
 ...
 */
    ...
    for (m = 1; m <= E->sphere.caps_per_proc; m++) {
      ...
      for (i = 1; i <= E->lmesh.nel; i++)
        fprintf(E->fp_out, "%d %d %f %f\n", i, E->mat[m][i],
                evisc[m][(i - 1) * vpts + 1], evisc[m][(i - 1) * vpts + 7]);
    }
    fflush(E->fp_out);
  }
  ...
  if (E->viscosity.allow_anisotropic_viscosity) {
    visc_from_gint_to_nodes(E, E->EVI2[E->mesh.levmax], E->VI2[E->mesh.levmax],
                            E->mesh.levmax);
    visc_from_gint_to_nodes(E, E->EVIn1[E->mesh.levmax],
                            E->VIn1[E->mesh.levmax], E->mesh.levmax);
    visc_from_gint_to_nodes(E, E->EVIn2[E->mesh.levmax],
                            E->VIn2[E->mesh.levmax], E->mesh.levmax);
    visc_from_gint_to_nodes(E, E->EVIn3[E->mesh.levmax],
                            E->VIn3[E->mesh.levmax], E->mesh.levmax);
    normalize_director_at_nodes(E, E->VIn1[E->mesh.levmax],
                                E->VIn2[E->mesh.levmax],
                                E->VIn3[E->mesh.levmax], E->mesh.levmax);

    if (E->viscosity.SMOOTH) {
      if (E->parallel.me == 0)
        fprintf(stderr, "WARNING: smoothing anisotropic viscosity, perhaps not "
                        "a good idea\n");
      visc_from_nodes_to_gint(E, E->VI2[E->mesh.levmax],
                              E->EVI2[E->mesh.levmax], E->mesh.levmax);
      visc_from_nodes_to_gint(E, E->VIn1[E->mesh.levmax],
                              E->EVIn1[E->mesh.levmax], E->mesh.levmax);
      visc_from_nodes_to_gint(E, E->VIn2[E->mesh.levmax],
                              E->EVIn2[E->mesh.levmax], E->mesh.levmax);
      visc_from_nodes_to_gint(E, E->VIn3[E->mesh.levmax],
                              E->EVIn3[E->mesh.levmax], E->mesh.levmax);
      normalize_director_at_gint(E, E->EVIn1[E->mesh.levmax],
                                 E->EVIn2[E->mesh.levmax],
                                 E->EVIn3[E->mesh.levmax], E->mesh.levmax);
    }
  }
#endif
...
void visc_from_mat(E, EEta) struct All_variables *E;
float **EEta;
{

  int i, m, jj;
  if (E->control.mat_control) { /* use pre-factor even without temperature
                                   dependent viscosity */
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= E->lmesh.nel; i++)
        for (jj = 1; jj <= vpoints[E->mesh.nsd]; jj++)
          EEta[m][(i - 1) * vpoints[E->mesh.nsd] + jj] =
              E->viscosity.N0[E->mat[m][i] - 1] * E->VIP[m][i];
  } else {
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= E->lmesh.nel; i++)
        for (jj = 1; jj <= vpoints[E->mesh.nsd]; jj++)
          EEta[m][(i - 1) * vpoints[E->mesh.nsd] + jj] =
              E->viscosity.N0[E->mat[m][i] - 1];
  }
...
void read_visc_layer_file(struct All_variables *E) {
  ...
  for (i = 0; i < E->mesh.elz; i++) {
    E->viscosity.N0[i] = E->viscosity.E[i] = E->viscosity.T[i] =
        E->viscosity.Z[i] = E->viscosity.pdepv_a[i] = E->viscosity.pdepv_b[i] =
            E->viscosity.pdepv_y[i] = E->viscosity.sdepv_expt[i] = 0;
  }
...
void visc_from_T(E, EEta, propogate) struct All_variables *E;
float **EEta;
int propogate;
{
  int m, i, k, l, z, jj, kk;
  ...
  float phi;
  float alpha = (float) E->mush.consts.visc_melt_coff;
    ...
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= nel; i++) {
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          EEta[m][(i - 1) * vpts + jj] = tempa * exp(-temp / E->viscosity.T[l]);
        }
      }
    break;

  case 3:
    /* eta = N_0 exp(E/(T+T_0) - E/(T_ref+T_0)) */
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= nel; i++) {
        l = E->mat[m][i] - 1;
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          EEta[m][(i - 1) * vpts + jj] =
              ...
              exp(E->viscosity.E[l] / (temp + E->viscosity.T[l]) -
                  E->viscosity.E[l] / (E->viscosity.T_ref + E->viscosity.T[l]));
        }
      }
    break;
    ...
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= nel; i++) {
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          EEta[m][(i - 1) * vpts + jj] =
              tempa * exp((E->viscosity.E[l] + E->viscosity.Z[l] * zzz) /
    ...
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= nel; i++) {
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          if (E->control.mat_control == 0) {
            EEta[m][(i - 1) * vpts + jj] =
                tempa * exp(E->viscosity.E[l] / (temp + E->viscosity.T[l]) -
                            E->viscosity.E[l] / (one + E->viscosity.T[l]));
          } else {
            visc2 = tempa * exp(E->viscosity.E[l] / (temp + E->viscosity.T[l]) -
                                E->viscosity.E[l] / (one + E->viscosity.T[l]));
            if (E->viscosity.MAX) {
    ...
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= nel; i++) {
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          /*
if(E->parallel.me == 0)
fprintf(stderr,"z %11g km mat %i N0 %11g T %11g T0 %11g E %11g Z %11g mat: %i
log10(eta): %11g\n", zzz *E->data.radius_km ,l+1,
         tempa,temp,E->viscosity.T[l],E->viscosity.E[l],
E->viscosity.Z[l],l+1,log10(EEta[m][ (i-1)*vpts + jj ]));
*/
        }
      }
    break;
      ...
      for (i = 1; i <= nel; i++) {
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          for (kk = 1; kk <= ends; kk++) {
            TT[kk] = max(TT[kk], zero);
            temp +=
                min(TT[kk], one) * E->N.vpt[GNVINDEX(kk, jj)]; /* mean temp */
...
*/
  ...
  case 30:
    /* eta(T) = N_0 exp(E/(T+T_0) - E/(T_ref+T_0)) */
    /* eta = eta(T) exp(-alpha * phi) where phi is melt fraction */
    /* (ZYZ-220215) */
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (i = 1; i <= nel; i++) {
        l = E->mat[m][i] - 1;
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          for (kk = 1; kk <= ends; kk++) {
            TT[kk] = max(TT[kk], zero);
            temp += min(TT[kk], one) * E->N.vpt[GNVINDEX(kk, jj)];
            phi += E->mush.comp.phi[E->ien[m][i].node[kk]] * E->N.vpt[GNVINDEX(kk, jj)];
          }
          ...
          EEta[m][(i - 1) * vpts + jj] = tempa * exp(E->viscosity.E[l] *
              (1 / temp - 1 / E->viscosity.T[l]) - alpha * phi);
          EEta[m][(i - 1) * vpts + jj] *= (1 - phi);
        }
      }
    break;
...
void visc_from_S(E, EEta, propogate) struct All_variables *E;
float **EEta;
int propogate;
{
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++) {
    if (E->viscosity.sdepv_visited) {
      ...
      for (e = 1; e <= nel;
           e++) /* initialize with unity if no velocities around */
        eedot[e] = 1.0;
    ...
    for (e = 1; e <= nel; e++) {
      ...
      for (jj = 1; jj <= vpts; jj++)
        EEta[m][(e - 1) * vpts + jj] =
            scale * pow(EEta[m][(e - 1) * vpts + jj], exponent1);
    }
  }
...
void visc_from_P(E,
                 EEta) /* "plasticity" implementation
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++) {
    ...
    if (E->viscosity.pdepv_visited) {
      if (E->viscosity.psrw)
        strain_rate_2_inv(
            E, m, eedot,
            0); /* get second invariant for all elements, don't take sqrt */
      else
        strain_rate_2_inv(E, m, eedot,
                          1); /* get second invariant for all elements */
    } else {
      for (e = 1; e <= nel;
           e++) /* initialize with unity if no velocities around */
        eedot[e] = 1.0;
      ...
      if ((E->parallel.me == 0) && (E->control.verbose)) {
        fprintf(stderr, "num mat: %i a: %g b: %g y: %g %s\n", e,
                E->viscosity.pdepv_a[e], E->viscosity.pdepv_b[e],
                E->viscosity.pdepv_y[e],
...
*/
      for (e = 1; e <= nel; e++) { /* loop through all elements */
        ...
        for (kk = 1; kk <= ends; kk++) /* nodal depths */
          zz[kk] =
              (1.0 -
               E->sx[m][3][E->ien[m][e].node[kk]]); /* for depth, zz = 1 - r */
        ...
        for (jj = 1; jj <= vpts; jj++) { /* loop through integration points */
          ...
          // fprintf(stderr,"z: %11g mat: %i a: %11g b: %11g y: %11g ee: %11g
          // tau: %11g eta_p: %11g eta_new: %11g eta_old: %11g\n",
          //	  zzz,l,E->viscosity.pdepv_a[l],
          // E->viscosity.pdepv_b[l],E->viscosity.pdepv_y[l],
      ...
      for (e = 1; e <= nel; e++) { /* loop through all elements */
        ...
        for (jj = 1; jj <= vpts; jj++) {
          ...
          if (tau < 1e10) {
            ...
            // fprintf(stderr,"SRW: a %11g b %11g y %11g z %11g sy: %11g e2:
            // %11g eold: %11g enew: %11g logr: %.3f\n",
...
*/
void visc_from_C(E, EEta) struct All_variables *E;
float **EEta;
{
...
void strain_rate_2_inv(E, m, EEDOT, SQRT) struct All_variables *E;
float *EEDOT;
int m, SQRT;
{
 ...
 */
      ...
      if ((e - 1) % E->lmesh.elz == 0) {
        construct_c3x3matrix_el(E, e, &E->element_Cc, &E->element_Ccx, lev, m,
                                1);
      }

      get_ba_p(&(E->N), GNx, &E->element_Cc, &E->element_Ccx, rtf, E->mesh.nsd,
               ba);
      ...
      for (j = 1; j <= ppts; j++) {
        for (i = 1; i <= ends; i++) {
          Vxyz[1][j] += (VV[1][i] * GNx->ppt[GNPXINDEX(0, i, j)] +
                        ...
                        rtf[3][j];
          Vxyz[2][j] +=
              ((VV[2][i] * GNx->ppt[GNPXINDEX(1, i, j)] +
                VV[1][i] * E->N.ppt[GNPINDEX(i, j)] * cos(rtf[1][j])) /
                   sin(rtf[1][j]) +
               VV[3][i] * E->N.ppt[GNPINDEX(i, j)]) *
              rtf[3][j];
          ...
          Vxyz[4][j] +=
              ((VV[1][i] * GNx->ppt[GNPXINDEX(1, i, j)] -
                VV[2][i] * E->N.ppt[GNPINDEX(i, j)] * cos(rtf[1][j])) /
                   sin(rtf[1][j]) +
          ...
          Vxyz[6][j] += VV[2][i] * GNx->ppt[GNPXINDEX(2, i, j)] +
                        rtf[3][j] * (VV[3][i] * GNx->ppt[GNPXINDEX(1, i, j)] /
                                         sin(rtf[1][j]) -
...
static void low_viscosity_channel_factor(struct All_variables *E, float *F) {
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++) {
    ...
    for (e = 1; e <= E->lmesh.elz; e++) {
      ...
      if (rad_mean >= E->viscosity.lv_min_radius)
        break;
    }
    nz_min[m] = e;
    ...
    for (e = E->lmesh.elz; e >= 1; e--) {
      rad_mean = 0.5 * (E->sx[m][3][E->ien[m][e].node[1]] +
                        E->sx[m][3][E->ien[m][e].node[8]]);
      if (rad_mean <= E->viscosity.lv_max_radius)
        break;
    }
    nz_max[m] = e;
  }
...
double second_invariant_from_3x3(double e[3][3]) {
  return (sqrt(0.5 * (e[0][0] * e[0][0] + e[0][1] * e[0][1] * 2.0 +
                      e[1][1] * e[1][1] + e[1][2] * e[1][2] * 2.0 +
                      e[2][2] * e[2][2] + e[0][2] * e[0][2] * 2.0)));
}
...
void get_vgm_p(double VV[4][9], struct Shape_function *N,
               struct Shape_function_dx *GNx, struct CC *cc, struct CCX *ccx,
               double rtf[4][9], int dims, int ppts, int ends, int spherical,
    ...
    for (a = 1; a <= ends; a++) {
      for (k = 1; k <= ppts; k++) {
        ...
        for (i = 1; i <= dims; i++) {
          ...
          /* velocity gradient matrix is transpose of grad v, using Citcom sort
t, p, r
...
*/
          ...
          vgm[0][1] =
              ((d_p * ut + shp * ccx->ppt[BPXINDEX(1, i, 2, a, k)]) * si[k] -
               shp * up * ct[k]) *
              ra[k];
          /* d_r vt = d_r v_t */
          vgm[0][2] = d_r * ut;
          /* d_t vp = 1/r d_t v_p*/
          vgm[1][0] =
              (d_t * up + shp * ccx->ppt[BPXINDEX(2, i, 1, a, k)]) * ra[k];
          /* d_p vp = 1/r((d_p vp)/sin(t) + vt/tan(t) + vr) */
          vgm[1][1] =
              ((d_p * up + shp * ccx->ppt[BPXINDEX(2, i, 2, a, k)]) * si[k] +
               shp * ut * ct[k] + shp * ur) *
              ra[k];
          /* d_r vp = d_r v_p */
          vgm[1][2] = d_r * up;
          ...
          vgm[2][1] =
              ((d_p * ur + shp * ccx->ppt[BPXINDEX(3, i, 2, a, k)]) * si[k] -
               shp * up) *
              ra[k];
          /* d_r vr = d_r vr */
          vgm[2][2] = d_r * ur;
...
*/
        l[0][0] += GNx->ppt[GNPXINDEX(0, a, k)] *
                   VV[1][a]; /* other contributions are zero */
