...
#define MAX_NCOMP 13  // ZYZ-220824
...
struct CAP {
  double heat_flux;// ZYZ-220218
  double melt_flux;// ZYZ-220412
  double theta[5];
  double fi[5];
  float *TB[4];
  float *VB[4];
...
struct MESH_DATA { /* general information concerning the fe mesh */
  ...
  int toplayerbc;     /* apply surface BC throughout top, or for a single internal
                         node */
...
struct HAVE { /* horizontal averages */
  float *T;
  float *V[4];
  float *vis;          // ZYZ-220221
  float *q;// ZYZ-220221
  float *phi;// ZYZ-220305
  float **C;
  float *c0;
  float *cl;
  float *E;
...
struct SLICE { /* horizontally sliced data, including topography */
  ...
  float *shflux[NCS];
  float *bhflux[NCS];
  float *smflux[NCS]; // melt flux (ZYZ-220412)
  float *bmflux[NCS]; // melt flux (ZYZ-220412)
  float *divg[NCS];
  float *vort[NCS];
  float *freesurf[NCS];
};
...
struct CONTROL {
  ...
  int precise_strain_rate; /* use proper computation for strain-rates in whole
                              domain, not just poles */
  ...
  char background_profile_file[1000];// ZYZ-220213
};
...
struct DATA {
  ...
  double ellipticity, ra, rc, rotm, j2, ge,
            efac; /* for ellipticity tests: f, normalized a and c axes,
...
/* for gzdir  */
struct gzd_struc {
  int vtk_io, vtk_base_init, vtk_base_save, vtk_ocount;
  float *vtk_base;
  FILE *vtk_fp;
...
/* constants and nondim values for CitcomS-mush */
struct MUSH_DATA {
  // melting information
  double latent_heat;
  ...
  int ncomp;
  double bulk_comp[MAX_NCOMP];
  double solid_density[MAX_NCOMP];
  double melt_density[MAX_NCOMP];
  double melting_point[MAX_NCOMP];
  double specific_gas_const[MAX_NCOMP];
  double melt_pressure_coff[MAX_NCOMP];
  double entropy_contrast;

  // viscosity information
  double visc_melt_coff;

  // dike information
  double tensile_strength;
  double extraction_coff;
...
/* composition field for CitcomS-mush (melting and crystallization) */
struct MUSH_COMP {
...
/* dynamic processes for CitcomS-mush
 * (mantle convection, darcy migration and dike extraction) */
struct MUSH_DYNAMICS {
  // mantle convection
  int convection_developed;
  int melt_buoyancy;
  double convection_developed_timestep;

  // darcy migration
  int darcy_migration;
  int solve_CMB_comp;
  int solid_compaction;
  int segregation_energy;
  double *q;

  // dike extraction
  int dike_extraction;
  int surface_downwelling;
  float extraction_limit;
  float downwelling_coff;
  double *E;
  double *M;
  double *Mn[MAX_NCOMP];
...
struct MUSH {
  ...
  // constants and nondim values
  struct MUSH_DATA consts;

  // composition field (melting and crystallization)
  struct MUSH_COMP comp;

  // dynamic processes (mantle convection, darcy migration and dike extraction)
  struct MUSH_DYNAMICS dynamics;
};
...
#include "viscosity_descriptions.h"
  ...
  struct MUSH mush; // CitcomS-mush

  higher_precision *Eqn_k1[MAX_LEVELS][NCS], *Eqn_k2[MAX_LEVELS][NCS],
      *Eqn_k3[MAX_LEVELS][NCS];
