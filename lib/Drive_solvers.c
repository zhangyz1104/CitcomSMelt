  ...
  if (E->control.NMULTIGRID || E->control.NASSEMBLE)
    ...
    for (i = E->mesh.gridmin; i <= E->mesh.gridmax; i++)
      for (m = 1; m <= E->sphere.caps_per_proc; m++)
        E->elt_k[i][m] =
            (struct EK *) malloc((E->lmesh.NEL[i] + 1) * sizeof(struct EK));
}
...
void general_stokes_solver(struct All_variables *E) {
  ...
  /* process melting before solving mantle convection */
  ...
  if (need_to_iterate(E)) {
    ...
    while (1) {
      ...
      if (E->parallel.me == 0) {
        fprintf(
            stderr,
            "Stress dep. visc./plast.: DUdot = %.4e (%.4e) for iteration %d\n",
  ...
  while (E->monitor.stop_topo_loop == 0) {
    ...
    if (E->viscosity.SDEPV || E->viscosity.PDEPV) {
      ...
      while (1) {
        ...
        if (E->parallel.me == 0) {
          fprintf(stderr,
                  "Stress dep. visc./plast.: DUdot = %.4e (%.4e) for iteration "
                  "%d\n",
