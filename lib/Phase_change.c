...
static void phase_change_apply(struct All_variables *E, double **buoy,
                               float **B, float **B_b, float Ra,
                               float clapeyron, float depth, float transT,
                               float inv_width);
static void calc_phase_change(struct All_variables *E, float **B, float **B_b,
                              float Ra, float clapeyron, float depth,
...
static void phase_change_apply(struct All_variables *E, double **buoy,
  ...
  if (E->control.verbose) {
    fprintf(E->fp_out,
            "Ra=%f, clapeyron=%f, depth=%f, transT=%f, inv_width=%f\n", Ra,
            clapeyron, depth, transT, inv_width);
    ...
    for (k = 1; k <= E->lmesh.noy; k++)
      for (j = 1; j <= E->lmesh.nox; j++) {
        ...
        for (i = 1; i < E->lmesh.noz; i++) {
          n = (k - 1) * E->lmesh.noz * E->lmesh.nox + (j - 1) * E->lmesh.noz +
              i;
          if (B[m][n] >= pt5 && B[m][n + 1] <= pt5)
            B_b[m][ns] = (E->sx[m][3][n + 1] - E->sx[m][3][n]) *
                             (pt5 - B[m][n]) / (B[m][n + 1] - B[m][n]) +
                         E->sx[m][3][n];
        }
      }
  }
