...
static void pg_solver(struct All_variables *E, double **T, double **Tdot,
                      double **DTdot, struct SOURCES *Q0, double diff, int bc,
                      unsigned int **FLAGS);
static void element_residual(struct All_variables *E, int el,
                             struct Shape_function *PG,
                             struct Shape_function_dx *GNx,
                             struct Shape_function_dA *dOmega, float VV[4][9],
                             double **field, double **fielddot,
                             struct SOURCES *Q0, double Eres[9],
                             double rtf[4][9], double diff, float **BC,
...
void advection_diffusion_parameters(struct All_variables *E) {
  ...
  input_int("adv_sub_iterations", &(E->advection.temp_iterations), "2,1,nomax",
            m);
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (el = 1; el <= nel; el++) {
      ...
      for (i = 1; i <= ENODES3D; i++) {
        ...
        uD += E->N.ppt[GNPINDEX(i, 1)] * E->mush.dynamics.q[E->ien[m][el].node[i]];
        uc3 += E->N.ppt[GNPINDEX(i, 1)] * VV[3][i];
      }
      uc = fabs(uc1) / E->eco[m][el].size[1] +
          fabs(uc2) / E->eco[m][el].size[2] +
          fabs(uc3) / E->eco[m][el].size[3];
      uD = fabs(uD) / E->eco[m][el].size[3];  // darcy flow
      uc = max(uc, uD);
...
void PG_timestep_solve(struct All_variables *E) {
  ...
  int i, m, psc_pass, iredo, j, isurf;
  ...
  /* check T for dike extraction downwelling */
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++)
      if (E->T[m][i] < E->control.TBCtopval)
        E->T[m][i] = E->control.TBCtopval;
  ...
  if (E->mush.dynamics.convection_developed > 0) // (ZYZ-220401)
  ...
  if (E->control.lith_age) {
    ...
    /* this is the old mode, it didn't quite work for me, so I'll not
       use it with ggrd */
    if (E->parallel.me == 0)
      fprintf(stderr, "PG_timestep_solve - lith_age adjust\n");
...
//      if (field[m][node] < 0) {
//        i = node;
//        isurf = ((i - 1) / E->lmesh.noz + 1) * E->lmesh.noz;
//        fprintf(stderr, "check T (0)[%d][%d]: T=%6.4g dT=%6.4g H=%6.4g E=%6.4g\n",
//                E->parallel.me, i, field[m][node], fielddot[m][node],
//                E->mush.dynamics.H[i], E->mush.dynamics.E[i]);
//        for (j = isurf; j > isurf - E->lmesh.noz; j--)
//          fprintf(stderr, "  node[%d]: c0=%6.4g phi=%6.4g T=%6.4g dT=%6.4g E=%6.4g V=%6.4g\n",
//                  isurf - j, E->mush.comp.c0[1][j], E->mush.comp.phi[j], field[m][j],
//                  fielddot[m][j] * E->advection.timestep, E->mush.dynamics.E[j], E->sphere.cap[m].V[3][j]);
//        fprintf(stderr, "\n");
//      }
...
static void pg_solver(struct All_variables *E, double **T, double **Tdot,
  ...
  double St, diff2;  // ZYZ modify SUPG
  float VV[4][9];
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (el = 1; el <= E->lmesh.nel; el++) {
      ...
      if (E->mush.mush_version)
        for (i = 1; i <= ends; i++)
          St = max(E->mush.comp.St[E->ien[m][el].node[i]], St);
      diff2 = diff / (1 + St);
      pg_shape_fn(E, el, &PG, &(E->gNX[m][el]), VV, rtf, diff2, m);

      element_residual(E, el, &PG, &(E->gNX[m][el]), &(E->gDA[m][el]), VV, T,
                       Tdot, Q0, Eres, rtf, diff, E->sphere.cap[m].TB, FLAGS,
                       m);
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++) {
      if (!(E->node[m][i] & (TBX | TBY | TBZ))) {
        DTdot[m][i] *= E->TMass[m][i];

        /* add heatflux of darcy flow and dike extraction */
        if (E->mush.mush_version)
          DTdot[m][i] += E->mush.dynamics.H[i] / (1 + E->mush.comp.St[i]);
      } else
        DTdot[m][i] = 0.0;
    }
...
void pg_shape_fn(struct All_variables *E, int el,
                 struct Shape_function *PG,
                 struct Shape_function_dx *GNx, float VV[4][9],
                 double rtf[4][9], double diffusion, int m) {
  int i, j;
  ...
  adiff = (unorm > 0.000001)
          ? ((uxse * xse + ueta * eta + ufai * fai) / (2.0 * unorm))
          : 0.0;
...
static void element_residual(struct All_variables *E, int el,
  ...
  double advection_term, diffusion_term, heating_term; // ZYZ-220227
  ...
  if (E->mush.mush_version)
    for (i = 1; i <= ends; i++) {
      St += E->mush.comp.St[E->ien[m][el].node[i]] * 0.125;
      c0 += E->mush.comp.c0[2][E->ien[m][el].node[i]] * 0.125;
    }

  /* heat production (ZYZ-220224) */
  if (Q0->number > 0) {
    Q = 0.0;
    for (i = 0; i < Q0->number; i++)
      Q += Q0->Q[i] *
          exp(-Q0->lambda[i] * (E->monitor.elapsed_time + Q0->t_offset));
  } else
    Q = E->control.Q0;

  if (E->mush.dynamics.inhomo_radio)
    Q *= c0 / E->mush.consts.bulk_comp[2];  // radioactive elements are related to anorthosite
  ...
  for (j = 1; j <= ends; j++) {
    ...
    for (i = 1; i <= vpts; i++) {
      ...
      diffusion_term = diff * dOmega->vpt[i] * E->heating_latent[m][el] *
          (GNx->vpt[GNVXINDEX(0, j, i)] * tx1[i] * rtf[3][i] +
              GNx->vpt[GNVXINDEX(1, j, i)] * tx2[i] * sint[i] +
              GNx->vpt[GNVXINDEX(2, j, i)] * tx3[i]);
      heating_term = PG->vpt[GNVINDEX(j, i)] * dOmega->vpt[i] * heating;

      Eres[j] += heating_term / (1 + St) - advection_term;
      if ((diffusion) && (E->mush.dynamics.convection_developed > 0))
        Eres[j] -= diffusion_term / (1 + St);
    }
  }
  ...
  if (FLAGS != NULL) {
    ...
    } else if (FLAGS[m][E->ien[m][el].node[5]] &
        FBZ) { // only check for the 5th node
      aid = 1;
      get_global_1d_shape_fn(E, el, &GM, &dGamma, aid, m);
    }
    if (aid >= 0) {
      for (a = 1; a <= onedvpts; a++) {
        ...
        for (j = 1; j <= onedvpts; j++) {
          ...
          for (k = 1; k <= onedvpts; k++)
            dT[j] += E->M.vpt[GMVINDEX(k, j)] *
                BC[3][E->ien[m][el].node[k + aid * onedvpts]];
        }
        for (j = 1; j <= onedvpts; j++) {
          Eres[a + aid * onedvpts] += dGamma.vpt[GMVGAMMA(aid, j)] *
              E->M.vpt[GMVINDEX(a, j)] *
              g_1d[j].weight[dims - 1] * dT[j];
        }
      }
    }
  }
...
static void filter(struct All_variables *E) {
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++) {
      ...
      /* remvoe undershoot */
      if (E->T[m][i] <= fabs(2 * Tmin0 - Tmin1))
        E->T[m][i] = Tmin0;
      if (E->T[m][i] >= (2 * Tmax0 - Tmax1))
        E->T[m][i] = Tmax0;
  ...
  MPI_Allreduce(&sum_rhocp, &total_sum_rhocp, 1, MPI_DOUBLE, MPI_SUM,
                E->parallel.world);
...
static void latent_heating(struct All_variables *E, int m,
                           ...
                           float **B, float Ra, float clapeyron, float depth,
                           float transT, float inv_width) {
  ...
  */
  temp0 = 2.0 * inv_width * clapeyron * E->control.disptn_number * Ra /
      E->control.Atemp / ends;
  ...
  for (e = 1; e <= E->lmesh.nel; e++) {
    ...
    for (i = 1; i <= ends; i++) {
      j = E->ien[m][e].node[i];
      temp = (1.0 - B[m][j]) * B[m][j] * (E->T[m][j] + E->control.surface_temp);
...
static void process_latent_heating(struct All_variables *E, int m,
  ...
  if (E->control.Ra_410 != 0.0) {
    latent_heating(E, m, heating_latent, heating_adi, E->Fas410,
                   E->control.Ra_410, E->control.clapeyron410,
                   E->viscosity.z410, E->control.transT410,
                   E->control.inv_width410);
  }

  if (E->control.Ra_670 != 0.0) {
    latent_heating(E, m, heating_latent, heating_adi, E->Fas670,
                   E->control.Ra_670, E->control.clapeyron670, E->viscosity.zlm,
                   E->control.transT670, E->control.inv_width670);
  }

  if (E->control.Ra_cmb != 0.0) {
    latent_heating(E, m, heating_latent, heating_adi, E->Fascmb,
                   E->control.Ra_cmb, E->control.clapeyroncmb,
                   E->viscosity.zcmb, E->control.transTcmb,
  ...
  if (psc_pass == (E->advection.temp_iterations - 1)) {
    ...
    if (E->parallel.me == 0) {
      fprintf(E->fp, "Step: %d, Total_heating(visc, adi): %g %g\n",
              E->monitor.solution_cycles, total_visc_heating,
              total_adi_heating);
...
void isotope_input(struct All_variables *E) {
  ...
  if (n > 0) {
    ...
    if (!input_float_vector("isotope_lambda", n,
                            E->convection.heat_sources.lambda, m)) {
