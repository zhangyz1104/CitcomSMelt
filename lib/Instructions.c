...
void initial_mesh_solver_setup(struct All_variables *E) {
  ...
  E->monitor.cpu_time_at_last_cycle = E->monitor.cpu_time_at_start =
      CPU_time0();
  ...
  (E->solver.parallel_domain_decomp0)(
      E); /* get local nel, nno, elx, nox et al */
  ...
  if (chatty)
    fprintf(stderr, "parallel setup done\n");

  /* physical domain */
  (E->solver.node_locations)(E);
  if (chatty)
    fprintf(stderr, "node locations done\n");

  allocate_velocity_vars(E);
  if (chatty)
    fprintf(stderr, "velocity vars done\n");
 ...
 already been initialized in GGrd_handling  */
  ...
  if (chatty)
    fprintf(stderr, "shape functions done\n");

  construct_elt_gs(E);
  if (chatty)
    fprintf(stderr, "construct elt gs done\n");
  ...
  mass_matrix(E);
  if (chatty)
    fprintf(stderr, "mass matrix done\n");
  construct_surf_det(E);
  construct_bdry_det(E);
  if (chatty)
    fprintf(stderr, "dets done\n");
  ...
  if (E->control.tracer) {
    ...
    if (chatty)
      fprintf(stderr, "tracer setup done\n");
  }
  ...
  if (chatty)
    fprintf(stderr, "initial_mesh_solver_setup done\n");
}
...
void initial_conditions(struct All_variables *E) {
  ...
  common_initial_fields(E); /* velocity/pressure/viscosity */
}
...
void read_initial_settings(struct All_variables *E) {
  ...
  /* Information on which files to print, which variables of the flow to
  calculate and print. Default is no information recorded (apart from special
  things for given applications.
 */
  ...
  if (E->control.CONJ_GRAD) {
    ...
    E->mesh.nox =
        E->mesh.mgunitx * (int) pow(2.0, levmax) * E->parallel.nprocx + 1;
    E->mesh.noy =
        E->mesh.mgunity * (int) pow(2.0, levmax) * E->parallel.nprocy + 1;
    E->mesh.noz =
        E->mesh.mgunitz * (int) pow(2.0, levmax) * E->parallel.nprocz + 1;
  }
 ...
 */
 ...
 r_grid_layers=3		# minus 1 is number of layers with uniform grid in
 r rr=0.5,0.75,1.0 	#    starting and ending r coodinates
 ...
 */
  ...
  input_int("num_mat", &(E->viscosity.num_mat), "1", m); /* number of layers,
                          moved from Viscosity_structures.c */
  if (E->viscosity.num_mat > CITCOM_MAX_VISC_LAYER)
    myerror(E, "too many viscosity layers as per num_mat, increase "
  ...
  input_float("z_lmantle", &(E->viscosity.zlm), "0.103594412180191",
              m); /*0.10359441  */
  input_float("z_410", &(E->viscosity.z410), "0.0643541045361796",
              m); /* 0.06434, more like it */
  input_float("z_lith", &(E->viscosity.zlith), "0.0156961230576048",
              m); /* 0.0157, more like it */
  ...
  input_float_vector("z_layer", E->viscosity.num_mat,
                     (E->viscosity.zbase_layer), m);
 ...
 */
  input_int("toplayerbc", &(E->mesh.toplayerbc), "0",
            m); /* > 0: apply surface boundary condition
 throughout all nodes with r > toplayerbc_r
 < 0: apply to single node layer noz+toplayerbc

 */
  input_float("toplayerbc_r", &(E->mesh.toplayerbc_r), "0.984303876942",
              m); /* minimum r to apply BC to,
 100 km depth */
  ...
  input_float("T_interior_max_for_exit", &(E->monitor.T_interior_max_for_exit),
              "1.5", m);

  input_int("pseudo_free_surf", &(E->control.pseudo_free_surf), "0", m);

  input_int("toptbc", &(E->mesh.toptbc), "1", m);
  input_boolean("toptbc_pole", &(E->mesh.toptbc_pole), "off",
                m); /* variable surface temp BC */
  ...
  input_int("mineral_physics_model", &(E->control.mineral_physics_model), "1",
            m);

  input_int("mat_control", &(E->control.mat_control), "0", m);
  input_string("mat_file", E->control.mat_file, "", m);

  input_boolean("precise_strain_rate", &(E->control.precise_strain_rate), "off",
                m);
 ...
 MATERIAL CONTROL
  ...
  read in time-constant prefactors from weak.grd netcdf file that apply to top
 two E->mat layers

  i.e.  ggrd_mat_control > 0 --> assign to layers with ilayer <=
 ggrd_mat_control ggrd_mat_control < 0 --> assign to layers with ilayer ==
 -ggrd_mat_control
 ...
 */
  ...
  /* time history file, if not specified, will use constant VBCs and material
   * grids */
  input_string("ggrd_time_hist_file", E->control.ggrd.time_hist.file, "", m);
  /* if > 0, will use top  E->control.ggrd.mat_control layers and assign a
   * prefactor for the viscosity */
  /* if < 0, will assign only to layer == -ggrd_mat_control */
  input_int("ggrd_mat_control", &(E->control.ggrd.mat_control), "0", m);
  input_boolean("ggrd_mat_limit_prefactor",
                &(E->control.ggrd_mat_limit_prefactor), "on",
                m); /* limit prefactor to with 1e+/-5 */
  input_int("ggrd_mat_is_code", &(E->control.ggrd_mat_is_code), "0", m); /* the
               viscosity grids are actually codes for different types of
  ...
  if (E->control.ggrd_mat_is_code) {
    ...
    E->control.ggrd_mat_code_viscosities =
        (float *)malloc(sizeof(float) * E->control.ggrd_mat_is_code);
    for (i = 0; i < E->control.ggrd_mat_is_code; i++)
      E->control.ggrd_mat_code_viscosities[i] = 1;
    input_float_vector("ggrd_mat_code_viscosities", E->control.ggrd_mat_is_code,
                       (E->control.ggrd_mat_code_viscosities), m);
  }
  input_string("ggrd_mat_file", E->control.ggrd.mat_file, "",
               m); /* file to read prefactors from */
  input_string("ggrd_mat_depth_file", E->control.ggrd_mat_depth_file,
               "_i_do_not_exist_", m);
  if (E->control.ggrd.mat_control !=
      0) /* this will override mat_control setting */
 ...
 */
  input_int("ggrd_rayleigh_control", &(E->control.ggrd.ray_control), "0", m);
  input_string("ggrd_rayleigh_file", E->control.ggrd.ray_file, "",
               m); /* file to read prefactors from */
  /*
 ...
 */
  ...
  input_string("ggrd_vtop_dir", E->control.ggrd.vtop_dir, "",
               m); /* file to read prefactors from */
 ...
 */
  ...
  input_boolean("ggrd_smooth_stages", &(E->control.ggrd_smooth_stages), "off",
                m);
  ...
  be set to free slip
 */
  input_boolean("allow_mixed_vbcs", &(E->control.ggrd_allow_mixed_vbcs), "off",
                m);
 ...
 */
  ...
  input_boolean("remove_rigid_rotation", &(E->control.remove_rigid_rotation),
                "on", m);
  input_boolean("inner_remove_rigid_rotation",
                &(E->control.inner_remove_rigid_rotation), "off", m);
  input_boolean("remove_angular_momentum",
                &(E->control.remove_angular_momentum), "on", m);

  input_boolean("self_gravitation", &(E->control.self_gravitation), "off", m);
  input_boolean("use_cbf_topo", &(E->control.use_cbf_topo), "off",
                m); /* make default on later XXX TWB */

  input_int("storage_spacing", &(E->control.record_every), "10", m);
  input_int("checkpointFrequency", &(E->control.checkpoint_frequency), "100",
            m);
  input_int("cpu_limits_in_seconds", &(E->control.record_all_until), "5", m);
  input_int("write_q_files", &(E->output.write_q_files), "0", m); /* write
                              additional heat flux files? */
  if (E->output.write_q_files) { /* make sure those get written at
                                 least as often as velocities */
    E->output.write_q_files =
        min(E->output.write_q_files, E->control.record_every);
  }
  ...
  input_double("inner_accuracy_scale", &(E->control.inner_accuracy_scale),
               "1.0,0.000001,1.0", m);

  input_boolean("force_iteration", &(E->control.force_iteration), "off", m);

  input_boolean("check_continuity_convergence",
                &(E->control.check_continuity_convergence), "on", m);
  input_boolean("check_pressure_convergence",
                &(E->control.check_pressure_convergence), "on", m);
  ...
  input_double("rayleigh", &(E->control.Atemp), "essential", m);
  ...
  if (E->control.inv_gruneisen != 0) {
    ...
    if (strcmp(E->control.uzawa, "cg") == 0) {
      ...
      input_int("compress_iter_maxstep", &(E->control.compress_iter_maxstep),
                "100", m);
 ...
 */
  ...
  if (fabs(E->data.rotm) > 5e-7) {
    /* J2 from flattening */
    E->data.j2 =
        2. / 3. * E->data.ellipticity * (1. - E->data.ellipticity / 2.) -
        E->data.rotm / 3. *
            (1. - 3. / 2. * E->data.rotm - 2. / 7. * E->data.ellipticity);
    /* normalized gravity at the equator */
    E->data.ge = 1 / (E->data.ra * E->data.ra) *
                 (1 + 3. / 2. * E->data.j2 - E->data.rotm);
    if (E->parallel.me == 0)
  ...
  E->data.ref_temperature =
      E->control.Atemp * E->data.therm_diff * E->data.ref_viscosity /
          (E->data.density * E->data.grav_acc * E->data.therm_exp) /
          (E->data.radius_km * E->data.radius_km * E->data.radius_km * 1e9);

  if (E->parallel.me == 0)
    fprintf(stderr, "Tref=%.4g\n", E->data.ref_temperature); // ZYZ-220410

  mush_input(E); // ZYZ-220223
  isotope_input(E); // ZYZ-220224
...
void check_settings_consistency(struct All_variables *E) {
  ...
  if (E->sphere.caps == 12 &&
    ...
    if (E->parallel.me == 0)
      fprintf(stderr,
              "\nWARNING: The input parameters impose boundary velocity, but "
              "also remove angular momentum/rigid rotation!\n\n");
  }
  ...
  if ((fabs(E->viscosity.zbase_layer[0] + 999) < 1e-5) &&
    ...
    if (E->viscosity.num_mat != 4)
      myerror(E, "error: either use z_layer for non dim layer depths, or set "
                 "num_mat to four");
    ...
    E->viscosity.zbase_layer[3] =
        E->viscosity.zcmb; /* the lowest layers is never checked, really
 ...
 */
  ...
  if (strcmp(E->output.vtk_format, "binary") == 0) {
    ...
    if (E->parallel.me == 0) {
      fputs("VTK binary output requires zlib, but couldn't find it at "
            "'configure' time. Please either use VTK ascii output or re-run "
            "'configure' with suitable flags.\n",
            stderr);
    }
    parallel_process_termination();
#endif
...
void global_derived_values(struct All_variables *E) {
  ...
  if (E->sphere.caps == 1) {
    ...
    /* number of nodes, excluding overlaping nodes between processors */
    /* each cap has one row of nox and one row of noy overlapped, exclude these
     * nodes. nodes at north/south poles are exclued by all caps, include them
     * by 2*noz*/
    E->mesh.nno =
        E->sphere.caps * (E->mesh.nox - 1) * (E->mesh.noy - 1) * E->mesh.noz +
            2 * E->mesh.noz;
  }
  ...
  /* Myr */
  E->data.scalet =
      (E->data.radius_km * 1e3 * E->data.radius_km * 1e3 / E->data.therm_diff) /
          (1.e6 * 365.25 * 24 * 3600);
  /* cm/yr */
  E->data.scalev = (E->data.radius_km * 1e3 / E->data.therm_diff) /
      (100 * 365.25 * 24 * 3600);
  E->data.timedir = E->control.Atemp / fabs(E->control.Atemp);

  if (E->control.print_convergence && E->parallel.me == 0) {
    fprintf(stderr,
            "Problem has nx %i x nz %i x ny %i nodes per cap, %i nodes and %i "
            "elements in total\n",
            E->mesh.nox, E->mesh.noz, E->mesh.noy, E->mesh.nno, E->mesh.nel);
    fprintf(E->fp,
            "Problem has %i x %i x %i nodes per cap, %i nodes and %i elements "
            "in total\n",
            E->mesh.nox, E->mesh.noz, E->mesh.noy, E->mesh.nno, E->mesh.nel);
  }
  return;
...

{
  ...
  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    ...
    E->slice.smflux[j] = (float *) malloc((nsf + 2) * sizeof(float));
    E->slice.bmflux[j] = (float *) malloc((nsf + 2) * sizeof(float));
  ...
  E->Have.q = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  E->Have.E = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  E->Have.M = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  E->Have.phi = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  E->Have.c0 = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  E->Have.cl = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  E->Have.drho2 = (float *) malloc((E->lmesh.noz + 2) * sizeof(float));
  ...
  for (i = E->mesh.levmin; i <= E->mesh.levmax; i++) {
    ...
    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      ...
      for (k = 1; k <= 4; k++)
        E->sphere.angle1[i][j][k] =
            (double *) malloc((snel + 1) * sizeof(double));

      E->GNX[i][j] = (struct Shape_function_dx *) malloc(
          (nel + 1) * sizeof(struct Shape_function_dx));
      E->GDA[i][j] = (struct Shape_function_dA *) malloc(
          (nel + 1) * sizeof(struct Shape_function_dA));
  ...
  for (i = 0; i <= E->output.llmax; i++)
    E->sphere.hindex[i] = (int *) malloc((E->output.llmax + 3) * sizeof(int));
  ...
  for (i = E->mesh.gridmin; i <= E->mesh.gridmax; i++)
    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      ...
      E->EVI[i][j] =
          (float *) malloc((nel + 1) * vpoints[E->mesh.nsd] * sizeof(float));
  ...
  if (E->viscosity.allow_anisotropic_viscosity) { /* any anisotropic
    ...
    for (i = E->mesh.gridmin; i <= E->mesh.gridmax; i++)
      for (j = 1; j <= E->sphere.caps_per_proc; j++) {
        ...
        E->EVI2[i][j] =
            (float *)malloc((nel + 1) * vpoints[E->mesh.nsd] * sizeof(float));
        E->avmode[i][j] = (unsigned char *)malloc(
            (nel + 1) * vpoints[E->mesh.nsd] * sizeof(unsigned char));
        E->EVIn1[i][j] =
            (float *)malloc((nel + 1) * vpoints[E->mesh.nsd] * sizeof(float));
        E->EVIn2[i][j] =
            (float *)malloc((nel + 1) * vpoints[E->mesh.nsd] * sizeof(float));
        ...
        if ((!(E->EVI2[i][j])) || (!(E->VI2[i][j])) || (!(E->EVIn1[i][j])) ||
            (!(E->EVIn2[i][j])) || (!(E->EVIn3[i][j])) || (!(E->VIn1[i][j])) ||
            (!(E->VIn2[i][j])) || (!(E->VIn3[i][j]))) {
          fprintf(stderr,
                  "Error: Cannot allocate anisotropic visc memory, rank=%i\n",
                  E->parallel.me);
          parallel_process_termination();
        }
      }
...

{
  ...
  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    ...
    for (i = 1; i <= E->mesh.nsd; i++) {
      E->sphere.cap[j].V[i] =
          (float *) malloc((E->lmesh.nnov + 1) * sizeof(float));
      E->sphere.cap[j].VB[i] =
          (float *) malloc((E->lmesh.nnov + 1) * sizeof(float));
      E->sphere.cap[j].Vprev[i] =
          (float *) malloc((E->lmesh.nnov + 1) * sizeof(float));
    }
  ...
  for (l = E->mesh.gridmin; l <= E->mesh.gridmax; l++)
    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      ...
      k = (E->lmesh.NOX[l] * E->lmesh.NOZ[l] +
          E->lmesh.NOX[l] * E->lmesh.NOY[l] +
          E->lmesh.NOY[l] * E->lmesh.NOZ[l]) *
          6;
...
void global_default_values(E)struct All_variables *E;
{
  ...
  E->control.precondition =
      0; /* for larger visc contrasts turn this back on  */
...
void set_up_nonmg_aliases(E, j)struct All_variables *E;
                               int j;
...
void report(E, string)struct All_variables *E;
                      char *string;
{
...
void record(E, string)struct All_variables *E;
                      char *string;
{
  ...
  while (1) {
    ...
    if (strcmp(prev, "connectivity") == 0)
      ...
      if (E->parallel.nprocxy != 12) {
        fprintf(stderr, "Warning: geoid calculation only works in full "
                        "version. Disabled\n");
      } else {
      ...
      /* Total temperature contrast is important when computing seismic
       * velocity,
...
static void chk_prefix(struct All_variables *E) {
  ...
  if (E->control.restart || E->control.post_p ||
    ...
    if (found) {
      fprintf(stderr,
              "error in input parameter: datafile_old='%s' contains '/'\n",
              E->control.data_file);
      parallel_process_termination();
    }
  }
...
static void expand_str(char *src, size_t max_size, const char *target,
                       const char *value) {
...
void output_init(struct All_variables *E) {
  ...
  if (strcmp(E->output.format, "ascii") == 0) {
    ...
    E->problem_output = vtk_output;
      ...
      /* indicate error here */
      if (E->parallel.me == 0) {
        fprintf(stderr, "wrong output_format, must be 'ascii', 'hdf5', "
                        "'ascii-gz' or 'vtk'\n");
        fprintf(E->fp, "wrong output_format, must be  'ascii', 'hdf5' "
                       "'ascii-gz', or 'vtk'\n");
      }
      parallel_process_termination();
    }
    ...
    /* indicate error here */
    if (E->parallel.me == 0) {
      fprintf(stderr, "wrong output_format, must be 'ascii', 'hdf5', or 'vtk' "
                      "(USE_GZDIR undefined)\n");
      fprintf(E->fp, "wrong output_format, must be 'ascii', 'hdf5', or 'vtk' "
                     "(USE_GZDIR undefined)\n");
    }
    parallel_process_termination();
  }
...
void print_all_config_parameters(struct All_variables *E) {
  ...
  if (E->parallel.me == 0) {
    ...
    fprintf(fp, "blob_center=[%g,%g,%g]\n", E->convection.blob_center[0],
            E->convection.blob_center[1], E->convection.blob_center[2]);
    ...
    if (E->viscosity.num_mat > 0) {
      for (i = 0; i < E->viscosity.num_mat - 1; i++)
        fprintf(fp, "%g,",
                (E->viscosity.SDEPV == 0 ? 1.0 : E->viscosity.sdepv_expt[i]));
      fprintf(fp, "%g\n",
              (E->viscosity.SDEPV == 0
               ? 1.0
               : E->viscosity.sdepv_expt[E->viscosity.num_mat - 1]));
    } else {
      fprintf(fp, "\n");
    }
    ...
    if (E->viscosity.num_mat > 0) {
      for (i = 0; i < E->viscosity.num_mat - 1; i++)
        fprintf(fp, "%g,",
                (E->viscosity.PDEPV == 0 ? 1e20 : E->viscosity.pdepv_a[i]));
      fprintf(fp, "%g\n",
              (E->viscosity.PDEPV == 0
               ? 1e20
               : E->viscosity.pdepv_a[E->viscosity.num_mat - 1]));
    } else {
      fprintf(fp, "\n");
    }
    fprintf(fp, "pdepv_b=");
    if (E->viscosity.num_mat > 0) {
      for (i = 0; i < E->viscosity.num_mat - 1; i++)
        fprintf(fp, "%g,",
                (E->viscosity.PDEPV == 0 ? 0.0 : E->viscosity.pdepv_b[i]));
      fprintf(fp, "%g\n",
              (E->viscosity.PDEPV == 0
               ? 0.0
               : E->viscosity.pdepv_b[E->viscosity.num_mat - 1]));
    } else {
      fprintf(fp, "\n");
    }
    fprintf(fp, "pdepv_y=");
    if (E->viscosity.num_mat > 0) {
      for (i = 0; i < E->viscosity.num_mat - 1; i++)
        fprintf(fp, "%g,",
                (E->viscosity.PDEPV == 0 ? 1e20 : E->viscosity.pdepv_y[i]));
      fprintf(fp, "%g\n",
              (E->viscosity.PDEPV == 0
               ? 1e20
               : E->viscosity.pdepv_y[E->viscosity.num_mat - 1]));
    } else {
      fprintf(fp, "\n");
    }
    ...
    fprintf(fp, "# new parameters of mushy MO\n"); // ZYZ-220213
    fprintf(fp, "background_profile_file=%g\n",
