...
static void solve_Ahat_p_fhat_CG(struct All_variables *E,
                                 ...
                                 double imp, int *steps_max) {
  int m, j, count, valid, lev, npno, neq, a, node;
  ...
  solid_compaction = E->mush.mush_version && E->mush.dynamics.solid_compaction &&
      (E->mush.dynamics.convection_developed > 0);
  ...
  /* CitcomS-mush: calculate the contribution of segregation flux in the continuity eqn */
  if (solid_compaction) {
    for (m = 1; m <= E->sphere.caps_per_proc; m++)
      for (j = 1; j <= npno; j++) {
        cu[m][j] = 0.0;
        for (a = 1; a <= 8; a++) {
          node = E->IEN[lev][m][j].node[a];
          cu[m][j] += E->elt_del[lev][m][j].g[3 * a - 1][0] * E->mush.dynamics.q[node];
        }
      }
  }
  ...
  if ((E->control.inv_gruneisen == 0) || !solid_compaction)
...
static void solve_Ahat_p_fhat_iterCG(struct All_variables *E,
  ...
  while ((relative_err_v >= imp || relative_err_p >= imp) &&
    ...
    if (E->parallel.me == 0) {
      fprintf(stderr,
              "itercg -- div(rho*v)/v=%.2e dv/v=%.2e and dp/p=%.2e loop %d\n\n",
              div_res,
              relative_err_v,
              relative_err_p,
              num_of_loop);
