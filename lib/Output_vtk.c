...
static void vts_file_header(struct All_variables *E, FILE *fp) {
  ...
  snprintf(extent, 64, "%d %d %d %d %d %d", E->lmesh.ezs,
           E->lmesh.ezs + E->lmesh.elz, E->lmesh.exs,
           E->lmesh.exs + E->lmesh.elx, E->lmesh.eys,
           E->lmesh.eys + E->lmesh.ely);
...
/* outut melt fraction (ZYZ-220315) */
static void vtk_output_melt_fraction(struct All_variables *E, FILE *fp) {
  int i;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *floattemp = malloc(nodes * sizeof(float));

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"melt_fraction\" "
          "format=\"%s\">\n",
          E->output.vtk_format);

  for (i = 0; i <= nodes; i++)
    floattemp[i] = (float) *(E->mush.comp.phi + i + 1);
...
/* outut bulk composition (ZYZ-220906) */
static void vtk_output_bulk_composition(struct All_variables *E, FILE *fp, int n) {
  int m, i;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *floattemp = malloc(nodes * sizeof(float));
  float *temp_node[NCS];
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    temp_node[m] = (float *) malloc((E->lmesh.nno + 1) * sizeof(float));

  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++)
      temp_node[m][i] = (float) E->mush.comp.c0[n][i];

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"composition_%d\" "
          "format=\"%s\">\n", n,
          E->output.vtk_format);

  for (i = 0; i <= nodes; i++)
    floattemp[i] = (float) *(temp_node[1] + i + 1);
...
/* outut latent heat (ZYZ-220315) */
static void vtk_output_latent_heat(struct All_variables *E, FILE *fp) {
  int i, j, e, m;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *floattemp = malloc(nodes * sizeof(float));
  float *temp_node[NCS];
  float dt = E->advection.timestep;
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++)
      temp_node[m][i] = E->mush.dynamics.H[i] * dt;

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"latent_heat\" "
          "format=\"%s\">\n",
          E->output.vtk_format);
...
static void vtk_output_extraction(struct All_variables *E, FILE *fp) {
  int i, m;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *floattemp = malloc(nodes * sizeof(float));
  float *temp_node[NCS];
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++)
      temp_node[m][i] = E->mush.dynamics.E[i] * dt;

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"dike_extraction\" "
          "format=\"%s\">\n",
          E->output.vtk_format);
...
static void vtk_output_emplacement(struct All_variables *E, FILE *fp) {
  int i, m;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *floattemp = malloc(nodes * sizeof(float));
  float *temp_node[NCS];
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++)
      temp_node[m][i] = E->mush.dynamics.M[i] * dt;

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"dike_emplacement\" "
          "format=\"%s\">\n",
          E->output.vtk_format);
...
/* outut latent heat (ZYZ-220315) */
static void vtk_output_density(struct All_variables *E, FILE *fp) {
  int i, m;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *temp_node[NCS];
  float *floattemp = malloc(nodes * sizeof(float));
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++)
      temp_node[m][i] = (float) E->mush.comp.drho[i];

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"phase_density_constrast\" "
          "format=\"%s\">\n",
          E->output.vtk_format);

  for (i = 0; i <= nodes; i++)
    // floattemp[i] = (float) *(E->mush.comp.drho + i + 1);
    floattemp[i] = (float) *(temp_node[1] + i + 1);
...
static void vtk_output_density2(struct All_variables *E, FILE *fp) {
  int i, m, noz;
  int nodes = E->sphere.caps_per_proc * E->lmesh.nno;
  float *temp_node[NCS];
  float *floattemp = malloc(nodes * sizeof(float));
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++)
    for (i = 1; i <= E->lmesh.nno; i++) {
      noz = (i - 1) % E->lmesh.noz + 1;
      temp_node[m][i] = (float) E->mush.comp.drho2[i] - E->Have.drho2[noz];
    }

  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"bulk_density_constrast\" "
          "format=\"%s\">\n",
          E->output.vtk_format);
...
static void vtk_output_diff_temp(struct All_variables *E, FILE *fp) {
  ...
  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"diff_temperature\" "
          "format=\"%s\">\n",
          E->output.vtk_format);
  compute_horiz_avg(E);
  for (i = 0; i <= nodes; i++) {
...
static void vtk_output_velo(struct All_variables *E, FILE *fp) {
  ...
  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    ...
    for (i = 1; i <= E->lmesh.nno; i++) {
      ...
      floatvel[(((j - 1) * E->sphere.caps_per_proc) + i - 1) * 3 + 0] =
          (float) (V[1][i] * cost * cosf - V[2][i] * sinf +
              V[3][i] * sint * cosf);
      floatvel[(((j - 1) * E->sphere.caps_per_proc) + i - 1) * 3 + 1] =
          (float) (V[1][i] * cost * sinf + V[2][i] * cosf +
              V[3][i] * sint * sinf);
      floatvel[(((j - 1) * E->sphere.caps_per_proc) + i - 1) * 3 + 2] =
          (float) (-V[1][i] * sint + V[3][i] * cost);

//      floatvel[(((j - 1) * E->sphere.caps_per_proc) + i - 1) * 3 + 0] = (float) V[1][i];
//      floatvel[(((j - 1) * E->sphere.caps_per_proc) + i - 1) * 3 + 1] = (float) V[2][i];
//      floatvel[(((j - 1) * E->sphere.caps_per_proc) + i - 1) * 3 + 2] = (float) V[3][i];
    }
  }
...
static void vtk_output_coord(struct All_variables *E, FILE *fp) {
  ...
  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    for (i = 1; i <= E->lmesh.nno; i++) {
      ...
      floatpos[((j - 1) * E->lmesh.nno + i - 1) * 3 + 1] =
          (float) (E->x[j][2][i]);
      floatpos[((j - 1) * E->lmesh.nno + i - 1) * 3 + 2] =
          (float) (E->x[j][3][i]);
    }
  }
...
static void vtk_output_stress(struct All_variables *E, FILE *fp) {
  ...
  fprintf(fp,
          "        <DataArray type=\"Float32\" Name=\"stress\" "
          "NumberOfComponents=\"6\" format=\"%s\">\n",
          E->output.vtk_format);
...
static void vtk_output_comp_nd(struct All_variables *E, FILE *fp) {
  ...
  for (k = 0; k < E->composition.ncomp; k++) {
    fprintf(fp,
            "        <DataArray type=\"Float32\" Name=\"composition%d\" "
            "format=\"%s\">\n",
            k + 1, E->output.vtk_format);

    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      for (i = 1; i <= E->lmesh.nno; i++) {
        floatcompo[(j - 1) * E->lmesh.nno + i - 1] =
            (float) (E->composition.comp_node[j][k][i]);
      }
    }
...
static void vtk_output_surf(struct All_variables *E, FILE *fp, int cycles) {
  ...
  if (E->control.use_cbf_topo) {
    ...
    get_STD_topo(E, E->slice.tpg, E->slice.tpgb, E->slice.divg, E->slice.vort,
                 cycles);
  }

  fprintf(
      fp,
      "        <DataArray type=\"Float32\" Name=\"surface\" format=\"%s\">\n",
      E->output.vtk_format);

  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    for (i = 1; i <= E->lmesh.nsf; i++) {
      for (k = 1; k <= E->lmesh.noz; k++) {
        floattopo[(j - 1) * E->lmesh.nno + (i - 1) * E->lmesh.noz + k - 1] =
            0.0;
      }
      ...
      if (E->parallel.me_loc[3] == E->parallel.nprocz - 1) {
        ...
        if (E->control.pseudo_free_surf)
          floattopo[(j - 1) * E->lmesh.nno + i * E->lmesh.noz - 1] =
              E->slice.freesurf[j][i];
        else
          floattopo[(j - 1) * E->lmesh.nno + i * E->lmesh.noz - 1] =
              E->slice.tpg[j][i];
      }
...
static void write_vtm(struct All_variables *E, int cycles) {
  ...
  snprintf(vtm_file, 255, "%s.%d.vtm", E->control.data_file, cycles);
...
static void write_visit(struct All_variables *E, int cycles) {
  ...
  snprintf(visit_file, 255, "%s.%d.visit", E->control.data_file, cycles);
...
static void write_pvts(struct All_variables *E, int cycles) {
  ...
  char pvts_file[255];
  int i, j, k;
  snprintf(pvts_file, 255, "%s.%d.pvts", E->control.data_file, cycles);
  ...
  snprintf(extent, 64, "%d %d %d %d %d %d", E->lmesh.ezs,
           E->lmesh.ezs + E->lmesh.elz * E->parallel.nprocz, E->lmesh.exs,
           E->lmesh.exs + E->lmesh.elx * E->parallel.nprocx, E->lmesh.eys,
           E->lmesh.eys + E->lmesh.ely * E->parallel.nprocy);
  ...
  if (E->output.comp_nd && E->composition.on) {
    fprintf(fp,
            "      <DataArray type=\"Float32\" Name=\"composition1\" "
            "format=\"%s\"/>\n",
            E->output.vtk_format);
  }
  if (E->output.surf) {
    ...
    for (j = 0; j < E->parallel.nprocx; j++) {
      for (k = 0; k < E->parallel.nprocz; k++) {
        fprintf(fp,
                ...
                "Source=\"%s.proc%d.%d.vts\"/>\n",
                (k % E->parallel.nprocz) * E->lmesh.elz,
                (k % E->parallel.nprocz + 1) * E->lmesh.elz,
                (j % E->parallel.nprocx) * E->lmesh.elx,
                (j % E->parallel.nprocx + 1) * E->lmesh.elx,
                (i % E->parallel.nprocy) * E->lmesh.ely,
                (i % E->parallel.nprocy + 1) * E->lmesh.ely,
                E->control.data_prefix,
                i * E->parallel.nprocx * E->parallel.nprocz +
                    j * E->parallel.nprocz + k,
                cycles);
      }
    }
  }
  ...
  case 6:
    for (i = 0; i < nn / 6; i++)
      fprintf(fp, "%.4e %.4e %.4e %.4e %.4e %.4e\n", array[6 * i],
              array[6 * i + 1], array[6 * i + 2], array[6 * i + 3],
              array[6 * i + 4], array[6 * i + 5]);
    break;
  }
  return;
}

static void FloatToUnsignedChar(float *floatarray, int nn,
...
static void zlibcompress(unsigned char *in, int nn, unsigned char **out,
                         int *nn2)
/* function to compress "in" to "out" reducing size from nn to nn2 */
{
  ...
  *out = malloc(sizeof(unsigned char) * nn);
  ...
  if (ret == Z_OK) {
    ...
    do {
      ...
      do {
        inz[j++] = in[i++];
               ...
               (i < nn)); // stopps if "inz"-buffer is full or "in" array empty
    ...
    fprintf(stderr, "Error during compression init\n");
  }
...
static void base64(unsigned char *in, int nn, unsigned char *out) {
  ...
  for (i = 0; i < nn; i += 3) {
    ...
    if (len >= 3) {
      ...
      out[i / 3 * 4 + 1] =
          cb64[((in[i] & 0x03) << 4) | ((in[i + 1] & 0xf0) >> 4)];
      out[i / 3 * 4 + 2] =
...
static void base64plushead(unsigned char *in, int nn, int orinn,
                           unsigned char *out) {
...
void vtk_output(struct All_variables *E, int cycles) {
  ...
  char output_file[255];
  FILE *fp;
  snprintf(output_file, 255, "%s.proc%d.%d.vts", E->control.data_file,
  ...
  E->output.surf = 0;
  output_horiz_avg(E, cycles); // ZYZ-220220
  output_S_H(E); // ZYZ-220323
  // output_surf_botm(E, cycles); // ZYZ-220415
  ...
  if (E->mush.mush_version) {
    vtk_output_melt_fraction(E, fp);
    vtk_output_density(E, fp);
    vtk_output_density2(E, fp);
    vtk_output_latent_heat(E, fp);
    vtk_output_extraction(E, fp);
    // vtk_output_emplacement(E, fp);
    for (n = 1; n <= E->mush.consts.ncomp; n++)
      vtk_output_bulk_composition(E, fp, n);
  }
