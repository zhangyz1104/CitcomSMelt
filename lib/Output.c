...
extern void get_STD_topo(struct All_variables *, float **, float **, float **,
                         float **, int);
...
void output_common_input(struct All_variables *E) {
  ...
  if (strcmp(E->output.format, "ascii-gz") == 0) {
    ...
    */
    ...
    E->output.gzdir.vtk_base_save =
        1; /* should we save the basis vectors? (memory!) */
...
void output_domain(struct All_variables *E) {
  ...
  const int corner_nodes[4] = {1, 1 + noz * (nox - 1),
                               nox * noy * noz - (noz - 1),
                               1 + noz * nox * (noy - 1)};
...
#define ncolumns 10
  ...
  if (E->parallel.me == 0) {
    ...
    for (rank = 1; rank < E->parallel.nproc; rank++) {
      MPI_Recv(buffer, ncolumns, MPI_DOUBLE, rank, tag, E->parallel.world,
               &status);
      fwrite(buffer, sizeof(double), ncolumns, fp1);
    }
...
void output_visc(struct All_variables *E, int cycles) {
  ...
  sprintf(output_file, "%s.visc.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");
...
void output_avisc(struct All_variables *E, int cycles) {
  ...
  if (E->viscosity.allow_anisotropic_viscosity) {
    sprintf(output_file, "%s.avisc.%d.%d", E->control.data_file, E->parallel.me,
            cycles);
    fp1 = output_open(output_file, "w");
    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      fprintf(fp1, "%3d %7d\n", j, E->lmesh.nno);
      for (i = 1; i <= E->lmesh.nno; i++)
        fprintf(fp1, "%.4e %.4e %.4e %.4e\n", E->VI2[lev][j][i],
                E->VIn1[lev][j][i], E->VIn2[lev][j][i], E->VIn3[lev][j][i]);
    }
...
void output_velo(struct All_variables *E, int cycles) {
  ...
  sprintf(output_file, "%s.velo.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");
  ...
  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    ...
    for (i = 1; i <= E->lmesh.nno; i++) {
      fprintf(fp1, "%.6e %.6e %.6e %.6e\n", E->sphere.cap[j].V[1][i],
              E->sphere.cap[j].V[2][i], E->sphere.cap[j].V[3][i], E->T[j][i]);
    }
  }
...
void output_surf_botm(struct All_variables *E, int cycles) {
  ...
  if (E->output.surf && (E->parallel.me_loc[3] == E->parallel.nprocz - 1)) {
    sprintf(output_file, "%s.surf.%d.%d", E->control.data_file, E->parallel.me,
            cycles);
    ...
    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      ...
      for (i = 1; i <= E->lmesh.nsf; i++) {
        s = i * E->lmesh.noz;
        fprintf(fp2, "%.4e %.4e %.4e %.4e %.4e\n", E->sx[j][1][s], E->sx[j][2][s], topo[i], E->slice.shflux[j][i],
                E->slice.smflux[j][i]);
      }
    }
    fclose(fp2);
  ...
  if (E->output.botm && (E->parallel.me_loc[3] == 0)) {
    sprintf(output_file, "%s.botm.%d.%d", E->control.data_file, E->parallel.me,
            cycles);
    fp2 = output_open(output_file, "w");
    ...
    for (j = 1; j <= E->sphere.caps_per_proc; j++) {
      ...
      for (i = 1; i <= E->lmesh.nsf; i++) {
        s = (i - 1) * E->lmesh.noz + 1;
        fprintf(fp2, "%.4e %.4e %.4e %.4e %.4e\n", E->slice.tpgb[j][i],
                E->slice.bhflux[j][i], E->slice.bmflux[j][i], E->sphere.cap[j].V[1][s],
                E->sphere.cap[j].V[2][s]);
      }
    }
    fclose(fp2);
  }
...
void output_geoid(struct All_variables *E, int cycles) {
  ...
  if (E->parallel.me == (E->parallel.nprocz - 1)) {
    sprintf(output_file, "%s.geoid.%d.%d", E->control.data_file, E->parallel.me,
            cycles);
    fp1 = output_open(output_file, "w");
...
void output_stress(struct All_variables *E, int cycles) {
  ...
  /*  */
  if (E->control.use_cbf_topo) { /* for CBF topo, stress will not have been
                                    computed */
  ...
  sprintf(output_file, "%s.stress.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");
  ...
  for (m = 1; m <= E->sphere.caps_per_proc; m++) {
    ...
    for (node = 1; node <= E->lmesh.nno; node++)
      fprintf(
          fp1, "%.4e %.4e %.4e %.4e %.4e %.4e\n",
          E->gstress[m][(node - 1) * 6 + 1], E->gstress[m][(node - 1) * 6 + 2],
          E->gstress[m][(node - 1) * 6 + 3], E->gstress[m][(node - 1) * 6 + 4],
          E->gstress[m][(node - 1) * 6 + 5], E->gstress[m][(node - 1) * 6 + 6]);
  }
  fclose(fp1);
}
...
void output_horiz_avg(struct All_variables *E, int cycles) {
  ...
  if (E->parallel.me < E->parallel.nprocz) {
    ...
    for (j = 1; j <= E->lmesh.noz; j++) {
      fprintf(fp1, "%.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e %.4e",
              E->sx[1][3][j], E->Have.T[j], E->Have.V[1][j], E->Have.V[2][j], E->Have.vis[j],
              E->Have.phi[j], E->Have.c0[j], E->Have.cl[j], E->Have.drho2[j],
              E->Have.q[j], E->Have.E[j], E->Have.M[j]);
...
void output_seismic(struct All_variables *E, int cycles) {
  ...
  void compute_seismic_model(struct All_variables *, double *, double *,
                             double *);
  ...
  sprintf(output_file, "%s.seismic.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
...
void output_tracer(struct All_variables *E, int cycles) {
  ...
  sprintf(output_file, "%s.tracer.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");
  ...
  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    fprintf(fp1, "%d %d %d %.5e\n", cycles, E->trace.ntracers[j], ncolumns,
            E->monitor.elapsed_time);

    for (n = 1; n <= E->trace.ntracers[j]; n++) {
      /* write basic quantities (coordinate) */
      fprintf(fp1, "%.12e %.12e %.12e", E->trace.basicq[j][0][n],
              E->trace.basicq[j][1][n], E->trace.basicq[j][2][n]);
...
void output_comp_nd(struct All_variables *E, int cycles) {
  ...
  sprintf(output_file, "%s.comp_nd.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");

  for (j = 1; j <= E->sphere.caps_per_proc; j++) {
    fprintf(fp1, "%3d %7d %.5e %d\n", j, E->lmesh.nel, E->monitor.elapsed_time,
            E->composition.ncomp);
    for (i = 0; i < E->composition.ncomp; i++) {
      fprintf(fp1, "%.5e %.5e ", E->composition.initial_bulk_composition[i],
...
void output_comp_el(struct All_variables *E, int cycles) {
  ...
  sprintf(output_file, "%s.comp_el.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");
...
void output_heating(struct All_variables *E, int cycles) {
  ...
  sprintf(output_file, "%s.heating.%d.%d", E->control.data_file, E->parallel.me,
          cycles);
  fp1 = output_open(output_file, "w");
...
/* ZYZ-220218 */
void output_heatflux(struct All_variables *E) {
  if (E->output.write_q_files > 0) {
    /* format: time heat_flow sqrt(v.v)  */
    if (E->parallel.me == E->parallel.nprocz - 1) {
      fprintf(E->output.fpqt, "%13.5e %13.5e %13.5e %13.5e\n", E->monitor.elapsed_time,
              E->sphere.cap[1].heat_flux, E->sphere.cap[1].melt_flux, sqrt(E->monitor.vdotv));
      fflush(E->output.fpqt);
    } else if (E->parallel.me == 0) {
      fprintf(E->output.fpqb, "%13.5e %13.5e %13.5e %13.5e", E->monitor.elapsed_time,
              E->sphere.cap[1].heat_flux, E->sphere.cap[1].melt_flux, sqrt(E->monitor.vdotv));
      if (E->mush.dynamics.solve_CMB_temp)
        fprintf(E->output.fpqb, " %13.5e", E->control.TBCbotval);
      fprintf(E->output.fpqb, "\n");
