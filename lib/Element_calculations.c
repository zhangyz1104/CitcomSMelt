    ...
    int lev, iconv;
{
  ...
  for (a = 1; a <= ends; a++)    /* loop over element nodes */
    for (b = a; b <= ends; b++) {
      ...
      // CitcomS-mush: solid compaction
      if (E->mush.mush_version && E->mush.dynamics.solid_compaction &&
      ...
      x[i] = E->GNX[lev][m][el].ppt[GNPXINDEX(2, a, 1)] * E->element_Cc.ppt[BPINDEX(3, i, a, 1)] +
          ...
          ra *
              (E->GNX[lev][m][el].ppt[GNPXINDEX(0, a, 1)] * E->element_Cc.ppt[BPINDEX(1, i, a, 1)] +
                  ...
                  si * (E->GNX[lev][m][el].ppt[GNPXINDEX(1, a, 1)]
                      * E->element_Cc.ppt[BPINDEX(2, i, a, 1)] +
...
static void get_elt_tr_pseudo_surf(struct All_variables *E,
                                   int bel,
                                   int side,
                                   double elt_tr[24],
                                   int m) {
  ...
  if (E->control.side_sbcs)
    ...
    if (side == SIDE_TOP && E->parallel.me_loc[3] == E->parallel.nprocz - 1
        && (el % E->lmesh.elz == 0)) {
      for (a = 1; a <= ends1; a++) {
        nodea = E->ien[m][el].node[sidenodes[side][a]];
        ...
        if (E->parallel.me == 11 && nodea == 3328)
          fprintf(stderr,
                  "traction=%e vnew=%e timestep=%e coeff=%e\n",
                  traction[3][a],
                  E->sphere.cap[m].V[3][nodea],
                  E->advection.timestep,
                  -1.0 * factor * rho * g * (R * R * R) / (eta * kappa));
        found = 1;
#if 0
