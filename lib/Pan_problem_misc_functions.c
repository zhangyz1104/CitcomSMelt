...
void get_buoyancy(struct All_variables *E, double **buoy) {
  ...
  const double B0 = E->mush.consts.B;
  ...
  /* melt-solid chemical buoyancy */
  if ((E->mush.mush_version) && (E->mush.dynamics.melt_buoyancy))
    for (i = 1; i <= E->lmesh.nno; i++) {
      // buoy[1][i] += B0 * temp * E->mush.comp.drho[i] * E->mush.comp.phi[i];
      buoy[1][i] += B0 * temp * E->mush.comp.drho2[i];
    }
  ...
  */
  ...
  if (isnan(buoy[1][i]) || isinf(buoy[1][i])) {
    fprintf(stderr, "\n\n\n--------debug information2---------\n");
    fprintf(stderr, "T=%.4e, phi=%.4e, Tsol=%.4e, Tliq=%.4e, c^ol=%.4e, c^an=%.4e, drho=%.4e, St=%.4e\n",
            E->T[1][i], E->mush.comp.phi[i], E->mush.comp.Tsol[i], E->mush.comp.Tliq[i], E->mush.comp.c0[1][i], E->mush.comp.c0[2][i], E->mush.comp.drho2[i], E->mush.comp.St[i]);
  }
